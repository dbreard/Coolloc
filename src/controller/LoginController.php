...
class LoginController extends Controller
{
    ...
    public function loginAction(Application $app, Request $request)
    {
        ...
        if (!empty($this->erreur)) { // si il y a des erreurs lors de la connexion, on les affiche
            return $app['twig']->render('formulaires/login.html.twig', array(
                "error" => $this->erreur,
            ));
        } else { // si les formats d'email et mdp sont bons, l'user se connecte et crée un token
            ...
            if (!empty($user)) { // Si la selection s'est bien passée
                if ($user['password'] == password_verify($password, substr($user['password'], 0, -32))) { // si les mot de passe cryptés correspondent
                    if($user['account'] === 'actif'){ // SI LE COMPTE DE L'UTILISATEUR EST BIEN ACTIF
                        ...
                        if (!empty ($resultatToken)) { // si le token existe, on récupère sa valeur dans la session
