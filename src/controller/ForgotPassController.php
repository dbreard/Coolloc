...
class ForgotPassController extends Controller
{
    ...
    public function forgotPassAction(Application $app, Request $request)
    {
        ...
        else
        {
            ...
            $userVerifEmail = $resultat->verifEmailBdd($email);
                ($userVerifEmail) ? $user = $resultat->verifEmailBdd($email) : array_push($this->erreur, 'L\'email n\'est pas associé à un compte ');
                if (!empty($user))
    ...
    // fonction qui vérifie le token et change le mdp
    public function verifEmailForgotAction (Application $app, Request $request){
        ...
        if(!$idUser) // SI AUCUN UTILISATEUR NE CORRESPOND AU TOKEN
            return $app['twig']->render('formulaires/forgotten-password.html.twig', array(
                "error" => 'Erreur lors de la validation de l\'email veuillez réessayer',
            ));
        else
        {
            $this->setToken($token);
            return $app->redirect("/Coolloc/public/change-password/{ " . $token . " }");
        }
        // var_dump($idUser);
        // die();
        
        // $updateUser = new UserModelDAO($app['db']); // MODIFICATION DU MDP UTILISATEUR
        // $rowAffected = $updateUser->modifyPasswordFromToken($idUser, $password);
